...
const getUserInfo = async (
  ...
  const { data } : { data: ApiResponse<UserInfo> } = await instance.get(`/api/v2/user/accounts`);
...
const verifyPassword = async () => {
  ...
  } = await axiosInstance.get("/api/v2/user/accounts/password/initialize");
...
const setPassword = async (body: SetPassword) => {
  const { data } = await axiosInstance.put("/api/v2/user/accounts/password/initialize", body);
  return validateApiResponse(data, "비밀번호 설정에 실패했습니다.");
};

// 비밀번호 변경
const changePassword = async (body: ChangePassword) => {
  const { data } = await axiosInstance.put("/api/v2/user/accounts/password", body);
  return validateApiResponse(data, "비밀번호 변경에 실패했습니다.");
};

// 회원 정보 수정
const updateUserInfo = async (body: UpdateUserInfo) => {
  const { data } = await axiosInstance.put("/api/v2/user/accounts", body);
...
const requestPhoneChangeCode = async (body: {
  ...
  } = await axiosInstance.post("/api/v2/user/accounts/phone-change/request-code", body);
...
const verifyPhoneChangeCode = async (body: VerifyPhoneChangeCode) => {
  const { data } = await axiosInstance.post("/api/v2/user/accounts/phone-change/verify-code", body);
  return validateApiResponse(data, "휴대폰번호 인증번호 검증에 실패했습니다.");
};

// SNS 연동 해제
const disconnectSns = async () => {
  const { data } = await axiosInstance.put("/api/v2/user/accounts/sns/disconnect");
  return validateApiResponse(data, "SNS 연동 해제에 실패했습니다.");
};

// 회원 탈퇴
const withdrawalAccount = async () => {
  const { data } = await axiosInstance.delete("/api/v2/user/accounts");
