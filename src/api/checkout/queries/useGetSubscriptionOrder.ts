...
import { getSubscriptionOrder } from "../checkout";

export function useGetSubscriptionOrder(
  subscribeId: number,
  queryOptions?: UseSuspenseQueryCustomOptions<SubscriptionOrderSheetResponse>
) {
  return useSuspenseQuery({
    queryFn: () => getSubscriptionOrder(subscribeId),
    queryKey: [
      queryKeys.ORDER.BASE,
      queryKeys.ORDER.GET_SUBSCRIPTION_ORDER,
      subscribeId,
    ],
