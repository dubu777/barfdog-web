...
export function useMultiFileUpload({
	...
	const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>(initialFilesRef.current);
		...
		try {
			...
			if (getExtraFormData) {
				...
				Object.entries(extra).forEach(([key, value]) => {
					...
					const appendValue = isBlob ? value : new Blob([JSON.stringify(value)], { type: 'application/json' });
	...
	const removeFile = async (fileId: number) => {
		try {
			// 서버에 삭제 요청
			if (deleteApiUrl) {
				const res = await uploadAxiosInstance.delete(`${deleteApiUrl}/${fileId}`);
				if (!res.data.success) {
