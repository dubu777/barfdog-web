...
const calculateOptionsTotal = (options: { count: number; price: number }[]) => {
  return options.reduce(
    (total, option) => total + option.count * option.price,
    0
  );
};
...
export const useStoreItemStore = create<StoreItemStore>((set) => ({
  ...
  setDiscountRate: (originalPrice, salePrice) =>
    set(() => ({
      salePrice,
      discountRate: calculateDiscountDegree(
        originalPrice,
        salePrice,
        "FIXED_RATE"
      ),
  ...
  addOption: (option) =>
    set((state) => {
      const existingOption = state.selectedOptions.find(
  ...
  updateOptionCount: (value, count) =>
    set((state) => {
      const updatedOptions = state.selectedOptions.map((option) =>
        option.value === value ? { ...option, count } : option
      );
      const optionsTotal = calculateOptionsTotal(updatedOptions);
      const newTotalPrice = state.itemPrice * state.itemAmount + optionsTotal;
      return {
  ...
  removeOption: (value) =>
    set((state) => {
      ...
      const newTotalPrice =
        state.itemPrice * state.itemAmount +
        calculateOptionsTotal(updatedOptions);
