...
const SUBSCRIPTION_ORDER_STATUS_LABEL: Record<string, string> = {
  ...Object.fromEntries(
    SUBSCRIPTION_ORDER_STATUSES.map((status) => [status, "진행중"])
  ),
...
const SUBSCRIPTION_ORDER_PROGRESS_BASE: Record<
  SubscriptionOrderStatus,
  Omit<OrderProgressInfo, "label">
> = {
  RESERVED_PAYMENT: {
    progress: 0,
  ...
  DELIVERY_DONE: {
    progress: 100,
    statusText: { payment: "결제완료", delivery: "배송완료" },
  },
  CONFIRM: {
    progress: 100,
    statusText: { payment: "결제완료", delivery: "배송완료" },
  ...
  SUBSCRIBE_PENDING: {
    progress: 0,
    statusText: { payment: "결제실패", delivery: "카드결제오" },
  },
  SUBSCRIBE_WILL_CANCEL: {
    progress: 0,
    statusText: { payment: "결제실패", delivery: "카드결제오류" },
  },
  SUBSCRIBE_CANCEL: {
    progress: 0,
...
const SUBSCRIPTION_ORDER_PROGRESS: Record<string, OrderProgressInfo> = {
  ...Object.fromEntries(
    Object.entries(SUBSCRIPTION_ORDER_PROGRESS_BASE).map(([key, value]) => [
      key,
...
function generateSubscriptionMessage(paymentDate: string, count: number) {
  return `${formatDate(
    paymentDate,
    "onlyDateDotKR"
  )} ${count}회차 정기구독이 시작될 예정입니다!`;
}
const SUBSCRIPTION_ORDER_STATUS_MESSAGES: Record<
  string,
  (paymentDate: string, count: number) => string
> = {
  ...
  DELIVERY_DONE: () =>
    "맛있는 식사가 배송되었습니다! 구매 확정 후 리뷰를 작성해보세요!",
  CONFIRM: () => "상품은 어떠셨나요? 리뷰 작성하고 포인트 혜택을 받아보세요!",
  SUBSCRIBE_PENDING: () =>
    "카드 정보를 수정해주세요. 2주 뒤 구독이 자동 해지됩니다.",
  SUBSCRIBE_WILL_CANCEL: () =>
    "카드 정보를 수정해주세요. 2주 뒤 구독이 자동 해지됩니다.",
  SUBSCRIBE_CANCEL: () =>
    "소중한 우리 아이의 영양관리를 위한 맞춤형 식단을 받아보세요!",
};
...
const MYPAGE_ITEM_TYPE_FILTERS: Record<
  keyof typeof ORDER_TYPE | "ALL",
  string
> = {
  ALL: "전체유형",
  GENERAL: "일반배송",
