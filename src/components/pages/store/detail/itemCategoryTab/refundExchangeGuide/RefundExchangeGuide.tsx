...
export default function RefundExchangeGuide() {
          ...
          text={
            <>
              ...
              <Text type="label3" color="red">
                (다음 주문의 배송 전 금요일 24시) 전 까지 고객님이 직접 취소
                가능
              </Text>
              <br />
              <br />
              바프독은 항상 목요일 주문마감, 금~일요일에 생산되어 화요일 일괄
              배송 됩니다
            </>
          }
        />
  ...
  return (
    ...
    >
      <ItemDetailLayout title="반품/교환 안내" hasTitlePadding={false}>
        <div
          className={commonWrapper({
            direction: "col",
            align: "start",
      ...
      <Divider height={2} color="gray200" />
      <ItemDetailLayout title="반품/교환 불가능 사유" hasTitlePadding={false}>
        <div
          className={commonWrapper({
            direction: "col",
            align: "start",
        ...
        >
          {reasonOfImpossibilityInfo.map((reason, index) => (
            <InfoText key={index} type="label2" color="gray800" text={reason} />
          ))}
        </div>
      </ItemDetailLayout>
      <Divider height={2} color="gray200" />
      <ItemDetailLayout title="판매자 정보" hasTitlePadding={false}>
        <div
          className={commonWrapper({
            direction: "col",
            align: "start",
