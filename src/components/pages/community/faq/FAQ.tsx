	...
	return (
		<>
			...
			>
				...
				>
					<Text type='title4'>보호자님들께서<br/>자주 하시는 질문을 모아봤어요!</Text>
					<Text type='body2' color='gray600'>이곳에 궁금하신 질문이 없다면 우측 하단의 상담 아이콘을 통해 실시간 상담 받아보세요!</Text>
				</article>
				<article className={commonWrapper({ backgroundColors: 'gray0', direction: 'col', align: 'start' })}>
					{FAQ_LIST.map((faq, index) => (
						<Accordion
							key={`${faq.label}${index}-${faq.question}`}
							contentClassName={commonWrapper({ backgroundColors: 'gray50' })}
							title={
								<div className={commonWrapper({ justify: 'start', align: 'center', gap: 20 })}>
									<Text type='label4' style={{ minWidth: '15%' }} block>{faq.label}</Text>
									<Text type='label3'>{faq.question}</Text>
								</div>
							}
						>
							<div className={commonWrapper({ direction: 'col', align: 'start', gap: 24 })}>
