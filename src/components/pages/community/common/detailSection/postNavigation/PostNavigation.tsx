  ...
  return (
    ...
    >
      ...
      >
        <Text type="headline1">
          {categoryLabel}{" "}
          <span className={pointColor}>{categoryPointLabel}</span> 게시글
        </Text>
        <button
          onClick={() => router.push(`/community/${category}`)}
          className={commonWrapper({ width: "auto", gap: 4 })}
        >
      ...
      {prevPost && (
        ...
        >
          <Text type="label3">{prevPost.title}</Text>
        </button>
      )}
      <button
        disabled
      ...
      {nextPost && (
        ...
        >
          <Text type="label3">{nextPost.title}</Text>
        </button>
      )}
      <Divider height={2} color="gray50" />
    </div>
