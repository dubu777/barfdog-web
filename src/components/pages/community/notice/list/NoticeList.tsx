...
export default function NoticeList() {
  ...
  const noticeTypeFilter =
    (searchParams.get("noticeType") as NoticeCategory) || "ALL";
  const noticeCategoryFilter = Object.entries(NOTICE_CATEGORY).map(
          ...
          {noticeList?.map((notice) => (
            <Fragment key={notice.id}>
              ...
              >
                ...
                {notice.createdDate && (
                  <Text type="label4">
                    {format(new Date(notice.createdDate), "yyyy-MM-dd")}
                  </Text>
                )}
              </Link>
