		...
		>
			<div className={styles.termsModalContainer}>
				...
				<div className={styles.termsModalContent}>
					...
					<br />
					- 회사의 서비스 이용 과정에서 서비스 이용기록, 방문기록,
					불량 이용기록, IP 주소, 쿠키, 광고식별자 등의 정보가 자동으로 생성되어 수집될 수 있습니다.
					<br />
					- 진행하는 이벤트에 따라 수집 항목이 상이할 수 있으므로
					응모 시 별도 동의를 받으며, 목적 달성 즉시 파기합니다.
					<br />
					<br />
					...
					후 별도의 DB로 옮겨져(종이의 경우 별도의 서류함) 내부 방침 및 기타 관련 법령에 의한
					...
					- 회사는 고객의 개인정보를 &apos;개인정보의 수집 및 이용목적&apos;에서 고지한 범위를 넘어 이용하거나
					타인 또는 타기업, 기관에 제공하지 않습니다. 2. 다음은 예외로 합니다.(가) 관계법령에 의하여
					수사상의 목적으로 관계기관으로부터의 요구가 있을 경우(나) 통계작성, 학술연구나 시장조사
					등을 위하여 특정 개인을 식별할 수 없는 형태로 광고주, 협력사나 연구단체 등에 제공하는
					경우(다) 기타 관계법령에서 정한 절차에 따른 요청이 있는 경우상기사항에 의해 개인정보를
					제공하는 경우에도 본래의 수집&middot;이용 목적에 반하여 무분별하게 정보가 제공되지 않도록 최대한
					노력하겠으며 보다 나은 서비스 제공, 고객편의 제공 등 원활한 업무 수행을 위하여 아래와 같이
					...
					공지사항을 통해 사전 공지합니다. 단기 이벤트는 참여 시에 개별 공지됩니다.
					<br />
					<br />
					...
					귀하의 컴퓨터 하드디스크에 저장됩니다. 회사은(는) 다음과 같은 목적을 위해 쿠키를
					사용합니다.
					<br />
					<p>
						쿠키 등 사용 목적
