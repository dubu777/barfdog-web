...
const PaymentMethodDetail = ({
  ...
  const cancelSubscription =
    subscriptionDetail.subscribeStatus === "SUBSCRIBE_WILL_CANCEL" ||
    subscriptionDetail.subscribeStatus === "SUBSCRIBE_CANCEL";

  const { addToast } = useToastStore();
  const {
    isOpen: isOpenChangeCard,
    onToggle: onIsOpenChangeCardToggle,
    onClose: onIsOpenChangeCardClose,
  } = useModal();
      ...
      <article className={styles.detailSubscription}>
        ...
        <div className={styles.detailSubscriptionList}>
          ...
          <ButtonDocked
            ...
            primaryButtonLabel={`결제정보${
              cancelSubscription ? "삭제" : "변경"
            }`}
