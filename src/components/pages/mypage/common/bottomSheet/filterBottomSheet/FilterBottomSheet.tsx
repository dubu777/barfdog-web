...
const FilterBottomSheet = ({ filters }: FilterBottomSheetProps) => {
  ...
  const [isOpenFilterOptions, setIsOpenFilterOptions] =
    useState<boolean>(false);

  // 초기 필터 값 설정 (첫 번째 값으로 기본 선택)
  const initialFilterValues: Record<string, string> = filters.reduce(
    (acc, filter) => {
  ...
  return (
    <Dropdown
      ...
      setIsOpen={setIsOpenFilterOptions}
      className={styles.filterBottomSheetContainer}
    >
      <BottomSheet
        isOpen={isOpenFilterOptions}
        onClose={() => setIsOpenFilterOptions(false)}
      >
        <div className={styles.filterBottomSheet}>
          ...
          <ul className={styles.filterBottomSheetOptions}>
            {filters.map((filter) => {
              ...
              const gridStyle = {
                gridTemplateColumns: `repeat(${optionGridColumns}, 1fr)`,
              };
              return (
                  ...
                  <div className={styles.filterOption} style={gridStyle}>
                    {Object.entries(filter.options).map(([value, label]) => (
                      <Button
                        ...
                        variant="outline"
                        intent={
                          tempFilterOptions[filter.key] === value
                            ? "primary"
                            : "assistive"
                        }
                        onClick={() =>
        ...
        <ButtonDocked
          type="full-button"
          primaryButtonLabel="조회"
          onPrimaryClick={handleFilterSubmit}
          position="sticky"
        />
      </BottomSheet>
    </Dropdown>
