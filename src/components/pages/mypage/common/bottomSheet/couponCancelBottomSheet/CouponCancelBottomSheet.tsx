...
import { calculateCouponDiscount, formatNumberWithCommas, isCouponUsable } from "@/utils";
...
const CouponCancelBottomSheet = ({
	...
	const usingDiscountCoupon = `${formatNumberWithCommas(usingCoupon.discount)}원`;
					...
					<div>
						...
						<ul className={styles.couponCardContainer}>
							{coupons.map(coupon => {
								...
								const { usable, reasons } = isCouponUsable(coupon, orderPrice, 'SUBSCRIBE');
