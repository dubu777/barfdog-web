...
import CouponCancelBottomSheet from "@/components/pages/mypage/common/bottomSheet/couponCancelBottomSheet/CouponCancelBottomSheet";
...
const SubscriptionPaymentInfo = ({
  ...
  const paymentPrice = hasUsingCoupon
    ? `${calcNextPaymentPrice.toLocaleString()}원`
    : `${data.nextPaymentPrice.toLocaleString()}원`;

  const isBlockedUsingCoupon = isNowBetweenMidnightAndPayment(
    "2025-04-23T23:27:25"
  );
  ...
  const paidInfo = [
    {
      ...
      value: "적용 필요",
      usingCouponValue:
        hasUsingCoupon && isBeforePaying ? { ...usingCouponValue } : null,
    },
    {
      label: "다음 회차",
  ...
  const maxAvailableCouponDiscount =
    data.nextPaymentPrice - IAMPORT_MIN_PAYMENT_PRICE;
  useEffect(() => {
  ...
  const handleUseCoupon = (selectedCoupon: {
    couponId: number;
  ...
  return (
    <>
      <InfoSection title="결제 예정 금액" isDefaultOpen gap={12}>
            ...
            return (
              <Fragment key={info.label}>
                <div className={styles.subscriptionPaymentItem}>
                  ...
                  <div
                    className={`${styles.subscriptionCardInfo} ${styles.subscriptionPaymentDiscount}`}
                  >
        ...
        {!isSubscriptionStatusWillCancel && (
          ...
          >
            {hasUsingCoupon
              ? "쿠폰 적용 변경"
              : `${
                  isBlockedUsingCoupon || isBeforePaying ? "이번" : "다음"
                } 회차에 쿠폰 적용`}
          </Button>
        )}
      </InfoSection>
      ...
      {isOpenCouponCancelBottomSheet && (
        <CouponCancelBottomSheet
          title={"먼저 적용 중인\n쿠폰을 취소해 주세요!"}
          subTitle={
            "이미 적용중인 쿠폰이 존재합니다. 쿠폰을 변경하고 싶으시다면\n현재 등록된 쿠폰의 적용을 취소해 주세요!"
          }
          isOpen={isOpenCouponCancelBottomSheet}
          onClose={onCloseCouponCancelBottomSheet}
