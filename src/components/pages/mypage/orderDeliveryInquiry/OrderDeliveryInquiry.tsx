...
const OrderDeliveryInquiry = () => {
  ...
  const { totalData, loadMore, hasNextPage, isFetchingNextPage } =
    useMergeOrderList({
      filterValue: searchParams.get("itemType") as keyof typeof ORDER_TYPE,
      statusFilter: "ORDER",
    });

  const orderData = totalData
    ?.map((data) => {
  ...
  const steps = [
    {
      key: "BEFORE_PAYMENT",
      label: "주문 접수",
      count: getStepsCount("BEFORE_PAYMENT"),
    },
    {
      key: "PAYMENT_DONE",
      label: "결제완료",
      count: getStepsCount("PAYMENT_DONE"),
    },
    {
      key: "DELIVERY_READY",
      label: "배송준비",
      count: getStepsCount("DELIVERY_READY"),
    },
    {
      key: "DELIVERY_START",
      label: "배송중",
      count: getStepsCount("DELIVERY_START"),
    },
    {
      key: "DELIVERY_DONE",
      label: "배송완료",
      count: getStepsCount("DELIVERY_DONE"),
    },
  ];
        ...
        {orderData.length < 1 ? (
          ...
          <div className={styles.orderList}>
            {orderData.map((orderData) => {
              return (
                <div
                  key={orderData?.orderId || orderData?.id}
                  className={styles.orderListItem}
                >
                  <Text type="caption" color="gray600">
                    주문일 {format(new Date(orderData.orderDate), "yy.MM.dd")}
                  </Text>
            ...
            <div
              ...
              style={{
                height: 50,
                background: isFetchingNextPage ? "lightgray" : "transparent",
              }}
            >
              {isFetchingNextPage
                ? "Loading more..."
