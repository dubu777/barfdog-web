'use client';
import * as styles from './SubscriptionBenefit.css';
...
import BenefitInfoList from "@/components/pages/mypage/subscriptionBenefit/benefitInfoList/BenefitInfoList";
import { BenefitDto } from "@/types/subscription";
import { useGetSubscriptionBenefits } from "@/api/subscription/queries/useGetSubscriptionBenefits";
...
const SubscriptionBenefit = ({ subscribeId }: { subscribeId: string }) => {
  const { data: subscriptionBenefits } = useGetSubscriptionBenefits(subscribeId);
  const [selectedBenefits, setSelectedBenefits] = useState<number[]>([]);

  const findBenefitItem = (benefitName: BenefitName) => subscriptionBenefits.find(benefit => benefit.benefitStatus === 'AVAILABLE' && benefit.benefitName === benefitName) || null;
