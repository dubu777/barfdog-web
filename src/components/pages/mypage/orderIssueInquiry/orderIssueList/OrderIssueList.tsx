...
const OrderIssueList = () => {
  ...
  const filteredIssueList = totalData
    .map((data) => {
      ...
      <Dropdown
        label={
          StatusTypeFilterList[
            searchParams.get("statusType") as keyof typeof StatusTypeFilterList
          ]?.label || "전체보기"
        }
        options={Object.entries(StatusTypeFilterList).map(
      ...
      <div className={styles.orderIssueList}>
        {filteredIssueList.map((data) => (
          <OrderIssueCard
            key={data.id}
            data={data}
            issueType={data.issueType as keyof typeof ORDER_ISSUE_TYPE}
