...
import ReviewNoticeBottomSheet from "@/components/pages/mypage/review/common/reviewNoticeBottomSheet/ReviewNoticeBottomSheet";
...
export default function ReviewForm({
  ...
  const { title, thumbnailUrl, imageUrl, orderedDate, writtenDate } =
    defaultReviewDetail;
  const { id: reviewId, reviewType } = reviewFormValues;

  const { control, handleSubmit, errors, isValid, setValue } =
    useFormHandler<ReviewFormValues>(
      reviewSchema,
      defaultReviewValue(reviewFormValues)
    );

  const star = useWatch({ control, name: "star" });

  const { uploadedFiles, uploadFile, removeFile, fileChangeInfo } =
    useMultiFileUpload({
      fileKey: "imageFile",
      idKey: "reviewId",
  ...
  return (
    <>
      ...
      >
        <article
          className={commonWrapper({ padding: 20, backgroundColors: "gray50" })}
        >
          <InfoBox
            text="리뷰 작성시 유의사항"
