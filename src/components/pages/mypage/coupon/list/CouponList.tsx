...
export default function CouponList() {
  ...
  const couponCategory =
    (searchParams.get("couponCategory") as CouponCategory) ?? "NON_ALLIANCE";

  const [sortBy, setSortBy] =
    useState<keyof typeof ItemSortByFilterList>("recent");
  const [couponCode, setCouponCode] = useState<string>("");
  const [couponCodeError, setCouponCodeError] = useState<string | null>(null);

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useGetInfiniteCouponList(couponCategory);
  const sortedCouponList = useFlattenedInfiniteData(data, "couponList", {
    sort: (a: Coupon, b: Coupon) => {
      ...
      if (a.discountType === "FIXED_RATE" && b.discountType === "FLAT_RATE")
        return -1;
      if (a.discountType === "FLAT_RATE" && b.discountType === "FIXED_RATE")
        return 1;
    ...
    <section>
      ...
      >
        <Text type="label4">
          사용가능한 쿠폰:{" "}
          {
            sortedCouponList.filter((coupon) => coupon.status === "ACTIVE")
              .length
          }
          개
        </Text>
        <Dropdown
          ...
          options={Object.entries(ItemSortByFilterList).map(
            ([value, { label }]) => ({
              label,
              value,
            })
          ...
          onSelect={(value) =>
            setSortBy(value as keyof typeof ItemSortByFilterList)
          }
        />
      </article>
