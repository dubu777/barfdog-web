...
import Image1 from "/public/images/main/service-1.png";
import Image2 from "/public/images/main/service-2.png";
import Image3 from "/public/images/main/service-3.png";
import Image4 from "/public/images/main/service-4.png";
    ...
    <article className={styles.mainServiceWrapper}>
      <div className={styles.mainServiceBox}>
        <h2
          ...
          style={{ marginBottom: isMobile ? '12px' : '4px' }}
        >
          내 가족이 먹는{isMobile && <br/>}안심 식단
        </h2>
        <p className={styles.mainDescription({ size: 'sm' })}>비교해 볼수록, 고민해 볼수록, 찾아볼수록,<br/>바프독은 최고의 서비스를 제공합니다</p>
      </div>
      <ul className={styles.mainServiceBox}>
        {servicesByBarfdogList.map(item => (
          <li key={item.key} className={styles.mainServiceListItem}>
            <h4 className={styles.mainTitle({ size: 'md' })}>
              {item.title}
            </h4>
            <p
              className={styles.mainDescription({ size: 'sm' })}
              style={{ margin: `${isMobile ? '12' : '15'}px 0 17px`, lineHeight: '20px' }}
            >
              {item.description}
            </p>
            <div>
      ...
      <p className={styles.mainDescription({ size: 'md', color: 'black', weight: 'bold' })}>
