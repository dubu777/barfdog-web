...
const Dogpedia = () => {
	...
	const [selectedDog, setSelectedDog] = useState<keyof typeof DOG_TYPE_TEMP| null>(null);
	const DOGPEDIA_OPTIONS = DOG_TYPE_OPTIONS.filter(v => v.value !== 'Unknown_Breed' && v.value !== 'Mixed_Breed');
	console.log();
		...
		<>
			{selectedDog === null ?
				<section className={styles.searchDogContainer}>
					<DefaultText type='title3'>궁금한 견종이 있으신가요?<br/>지금 바로 검색해 보세요</DefaultText>
						...
						<SearchableSelector
							...
							emptyElement={(
								<div className={styles.searchableSelectorEmpty}>
									<DefaultText type='label1' color='gray700' align='center'>앗, 현재 등록되지 않은 견종이에요!<br/>다른 견종을 검색해 보세요!</DefaultText>
								</div>
							)}
						/>
					</div>
				...
				: (
					<DogpediaDetail selectedDog={selectedDog} setSelectedDog={setSelectedDog} />
				)
			}
		</>
	);
