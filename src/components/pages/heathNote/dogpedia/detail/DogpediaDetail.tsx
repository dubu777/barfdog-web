...
interface DogpediaDetailProps {
	setSelectedDog: (selectedDog: keyof typeof DOG_TYPE | null) => void;
}
...
const dummyData = {
	...
	nutritionIntake: "고든 세터에게 단백질 함량이 높은(26% 이상) 식단을 먹이는 것은 권장되지 않습니다. 급격한 성장을 강요하면 건강 문제가 발생할 수 있기 때문입니다. 단단한 변을 위해서는 섬유질 함량이 최소 4% 이상인 것이 가장 좋습니다. 고품질 건사료, 습식사료, 그리고 원한다면 닭고기, 연어, 과일, 조리된 신선한 야채와 같은 소량의 건강에 좋은 첨가물을 먹이십시오."
}
...
const DogpediaDetail = ({
	...
	const defaultInfoData = {
		dogType: dummyData.dogType,
		minWeight: dummyData.minWeight,
		maxWeight: dummyData.maxWeight,
		minHeight: dummyData.minHeight,
		maxHeight: dummyData.maxHeight,
		lifeExpectancy: dummyData.lifeExpectancy,
		keyword: dummyData.keyword,
		diseases: [
			dummyData.diseasesFirst,
			dummyData.diseasesSecond,
			dummyData.diseasesThird,
		],
	}

	const detailCategoryTabData = {
		familyAffection: dummyData.familyAffection,
		childrenInteraction: dummyData.childrenInteraction,
		friendliness: dummyData.friendliness,
		shedding: dummyData.shedding,
		groomingFrequency: dummyData.groomingFrequency,
		droolingLevel: dummyData.droolingLevel,
		coat_type: dummyData.coat_type,
		coat_length: dummyData.coat_length,
		sociability: dummyData.sociability,
		playfulness: dummyData.playfulness,
		guardingStance: dummyData.guardingStance,
		adaptability: dummyData.adaptability,
		trainability: dummyData.trainability,
		activityLevel: dummyData.activityLevel,
		barkingFrequency: dummyData.barkingFrequency,
		mentalStimulationNeeds: dummyData.mentalStimulationNeeds,
		furType: dummyData.furType,
		furLength: dummyData.furLength,
	}

	const detailInfoData = {
		health: dummyData.health,
		recommendedHealthTests: dummyData.recommendedHealthTests,
		grooming: dummyData.grooming,
		exercise: dummyData.exercise,
		training: dummyData.training,
		nutritionIntake: dummyData.nutritionIntake,
	}
	return (
		<>
