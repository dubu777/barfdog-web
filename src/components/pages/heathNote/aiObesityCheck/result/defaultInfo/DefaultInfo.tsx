	...
	return (
		<div className={commonWrapper({
			...
			<div className={commonWrapper({ direction: 'col', gap: 8 })}>
				<ResultTitle title={`${format(new Date(data.regDate), 'yyyy-MM-dd')} 비만 AI 진단 결과`} />
			...
			>
				<Text type='headline3' align='center' className={commonWrapper({ gap: 8 })}>
					<SvgIcon src={WeightIcon} size={32} />
					<span>
						{data.score}점으로{' '}
						<span className={pointColor}>{data?.status}</span> 
						{' '}결과를 받았어요
					</span>
				</Text>
				<Card 
				...
				>
					<div className={commonWrapper({
						...
						<Divider direction='vertical' thickness={1} color='gray300' style={{ height: '50px' }} />
				...
				>
					{imageList?.map(image => (
						<SwiperSlide key={image.filename} className={commonWrapper({ paddingBottom: 30 })}>
							<Image 
								src={image.url} 
								alt={image.filename} 
								width={500} 
								height={500} 
								className={imageWrapper({ height: 'auto', objectFit: 'cover', borderRadius: 16 })} 
							/>
						</SwiperSlide>
					))}
				</Swiper>
