...
const REQUEST_FORM_NOTICE_LIST = [
  {
    value: (
      <DefaultText type="body3" color="gray700">
        회수 신청 후 문 앞에 채취한 변이 담긴 키트를 포장하여 놓아주시면 수거 후
        분석기관에 전달됩니다.
      </DefaultText>
    ),
    isPointColor: false,
  ...
  },
  {
    value: (
      <div>
        ...
        <DefaultText type="body3" color="gray700">
          입니다. 이후 신청하는 회수 신청 건은 차주 월요일 회수 진행될
          예정입니다.
        </DefaultText>
      </div>
    ),
  ...
  },
  {
    value: (
      <div>
        <DefaultText type="headline4" color="red">
          분석 결과는 키트 회수 시점 기준 평균 4~6주 후에{" "}
        </DefaultText>
        <DefaultText type="body3" color="gray700">
          받아보실 수 있습니다. 보다 정확한 분석 결과를 제공드리기 위해 다소
          오랜 시간이 소요되는 점 양해 말씀드립니다.
        </DefaultText>
      </div>
    ),
    isPointColor: true,
...
const RequestForm = ({ confirm, setConfirm }: RequestFormProps) => {
  ...
  const handleFileUpload = async (files: ImageFile[]) => {
    ...
    setAddImageIdList((prev) =>
      Array.from(new Set([...prev, ...uploadedImageList]))
    );
  };
