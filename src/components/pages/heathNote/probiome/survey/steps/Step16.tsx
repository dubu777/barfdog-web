...
export default function ProbiomeStep16({
  handleChange,
  dogName,
}: SurveyStepProps) {
  ...
  return (
    <>
      <SurveyTitle dogName={dogName} config={PROBIOME_TITLES.step16} />
      <div
      ...
      >
        {PROBIOME_FORM_INFO.additionalInfo.acquisitionType.options.map(
          (option) => (
            <SurveyButton
              key={option.label}
              label={option.label}
