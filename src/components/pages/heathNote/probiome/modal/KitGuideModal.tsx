...
import KitGuidStep3 from "/public/images/healthNote/probiome/kit-guide-step3.svg";
import KitGuidStep4 from "/public/images/healthNote/probiome/kit-guide-step4.svg";
import KitGuidStep5 from "/public/images/healthNote/probiome/kit-guide-step5.svg";
...
const KIT_GUIDE_STEPS = [
  ...
  {
    title:
      "분변 채취 전 대변을 촬영하여 키트 회수 신청 시 함께 업로드해 주세요",
    description: "업로드 시 분석 정확도가 올라가요",
    image: <KitGuidStep2 />,
  },
  {
    title:
      "배설 후 30분 이내로 변에 묻은 이물질 등은 털고 반을 잘라 안쪽 부분을 용액 통 뚜껑에 붙은 스푼으로 떠 주세요",
    description: "변 양은 용액이 넘치지 않고 용액에 잠길 정도로만 떠주세요",
    image: <KitGuidStep3 />,
  },
  {
    title:
      "스푼을 그대로 용액에 넣고 꽉 잠근 후 좌우로 10회 정도 가볍게 흔들어 보존액을 섞어 주세요",
    description: "너무 과하게 흔들지 말아주세요",
    image: <KitGuidStep4 />,
  },
  {
    title:
      "채변통 부착 스티커에 정보를 작성하여 채변 용기에 부착한 후 회수용 봉투에 담아 주세요",
    description: "용기 뚜껑이 잘 잠겼는지 확인 꼭!",
    image: <KitGuidStep5 />,
  },
  {
    title:
      "회수 신청(온라인/QR코드) 후 회수 봉투에 담은 후 회수 스티커 부착, 문 앞에 내놓으면 수거 완료!",
      ...
      <article className={styles.kitGuideNotice}>
        ...
        <Card shadow="none" gap={8} backgroundColor="gray50" padding={16}>
          {KIT_GUIDE_NOTICE_LIST.map((notice, index) => (
            <InfoText
              key={index}
              color={notice.isPointColor ? "red" : "gray700"}
              type="body3"
            >
