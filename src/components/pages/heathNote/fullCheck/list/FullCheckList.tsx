		...
		<section>
			<div className={styles.fullCheckSurvey}>
				<Card shadow='light' className={styles.fullCheckSurveyCard}>
					<div className={styles.fullCheckSurveyCardContent}>
						<DefaultText type='headline2'>우리 아이 건강,<br/>한번 더 살펴볼까요?</DefaultText>
						<DefaultText type='caption2' color='gray600'>건강 종합 진단으로 건강 변화를 쉽게 기록할 수 있어요</DefaultText>
						<button onClick={handleGoToSurvey} className={styles.fullCheckSurveyButton}>
							<DefaultText type='headline4' color='red'>진단하러 가기</DefaultText>
			...
			<article className={styles.fullCheckResultList}>
				{healthCheckResults.map((result, index) => (
					<Link href={`/health-note/full-check/result/${result.id}?score=${result.score}`} key={index}>
						<Card shadow='light' padding={16}>
							<DefaultText type='label4' color='gray600'>{result.createdDate}</DefaultText>
							<div className={styles.resultTopRank}>
								<DefaultText type='headline2'>{dogInfo?.name}의 검사결과</DefaultText>
								<Chips variant='solid' borderRadius='lg' color='gray200'>상위{result.rank}%</Chips>
							</div>
