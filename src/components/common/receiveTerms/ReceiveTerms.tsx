...
import { Control, Controller, Path, PathValue, UseFormSetValue, UseFormWatch } from "react-hook-form";
import { UpdateUserInfo, SignUpFormValues } from "@/types";
...
const ReceiveTerms = <T extends boolean>({
	...
	const receiveSms = isSignUp ? watch('agreement.receiveSms' as Path<FormValues<T>>): watch('receiveSms' as Path<FormValues<T>>)
	const receiveEmail = isSignUp ? watch('agreement.receiveEmail' as Path<FormValues<T>>): watch('receiveEmail' as Path<FormValues<T>>)
	...
	const handleAllReceiveChange = (checked: boolean) => {
		...
		if (isSignUp) {
			const currentAgreement = watch('agreement' as Path<FormValues<T>>) as FormValues<T>;
			setValue('agreement' as Path<FormValues<T>>, {
				...currentAgreement,
				receiveSms: checked,
				receiveEmail: checked,
			} as PathValue<FormValues<T>, Path<FormValues<T>>>);
		} else {
			setValue('receiveSms' as Path<FormValues<T>>, checked as PathValue<FormValues<T>, Path<FormValues<T>>>);
			setValue('receiveEmail' as Path<FormValues<T>>, checked as PathValue<FormValues<T>, Path<FormValues<T>>>);
		}
	}
	...
	return (
		<div className={styles.receiveList}>
			...
			<div className={styles.receiveAgreement}>
				<Controller
					name={isSignUp ? 'agreement.receiveSms' as Path<FormValues<T>> : 'receiveSms' as Path<FormValues<T>>}
					control={control}
					render={({ field }) => (
						<DefaultCheckbox
							{...field}
				...
				/>
				<Controller
					name={isSignUp ? 'agreement.receiveEmail' as Path<FormValues<T>> : 'receiveEmail' as Path<FormValues<T>>}
					control={control}
					render={({ field }) => (
						<DefaultCheckbox
							{...field}
