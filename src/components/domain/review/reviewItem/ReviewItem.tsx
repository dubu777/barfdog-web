...
export default function ReviewItem({
  ...
  const reviewImageList =
    data?.reviewImageList.map((image) => ({ ...image, fileName: "" })) || [];
  ...
  return (
    <>
      <div className={contentBox({ background: backgroundColor, isExpanded })}>
        ...
        />
        {isExpanded && (
          <Divider height={1} color="gray200" direction="horizontal" />
        )}
        <div
        ...
        >
          ...
          >
            {!isExpanded && hasReviewImages && (
              <SvgIcon src={PictureIcon} size={24} />
            )}
            <Text
              type="body2"
              className={!isExpanded ? ellipsis({ lineSize: "line1" }) : ""}
            >
              {contents}
            </Text>
          </div>
